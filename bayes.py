# -*- coding: utf-8 -*-
"""
Created on Tue Apr 10 23:02:40 2018
...
def textParse(bigString):
    ...
    return[tok.lower for tok in listOfTokens if len(tok)>2]#使得内容全部小写
    
def spamTest():
    docList = [];classList = []; fullText=[]
    for i in range(1,26):
        wordList = textParse(open('G:/machine_learning/Ch04/email/spam/%d.txt'%i).read())
        docList.append(wordList)
        fullText.extend(wordList)
        classList.append(1)
        wordList = textParse(open('G:/machine_learning/Ch04/email/ham/%d.txt'%i).read())
        docList.append(wordList)
        fullText.extend(wordList)
        classList.append(0)
...
def localWords(feed1,feed0):
    ...
    for pariw in top30Words:
        if parirW[0] in vocabList: vocabList.remove(pairW[0])
...
def getTopWords(ny,sf):
    ...
    for i in range(len(p0V)):
        ...
        if p0V[i]>-6.0: topNY.append((vocabList[i],p1V[i]))
